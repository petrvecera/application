...
router.post('/company/export-leave-summary/', function (req, res) {

  var month_from = moment(req.param('from_date'), 'DD/MM/YYYY'), // TODO: Format is hardcoded!
    month_to = moment(req.param('to_date'), 'DD/MM/YYYY'), // TODO: Format is hardcoded!
    months_to_process = [],
    sheets = []; // The excel sheet can have multiple sheets for each month

  req.user
    .get_company_for_export()
    .then(function (company) {

      // Prepare the columns of the sheet / aka layout
      var columns = [{
        caption: 'Employee',
        type: 'text',
        width: 20
      ...
      for (var month = moment(month_from); month.isSameOrBefore(month_to, 'month'); month.add(1, 'M')) {
        months_to_process.push(moment(month));
      }

      var promises_user_leave_summaries = [];

      months_to_process.forEach(function (month) {
        var layout = {
        ...
        company.users.forEach(function (iterate_user) {
          promises_user_leave_summaries.push(
            iterate_user.promise_my_leave_summary_for_month({month: month}).then(function (summary) {
              var row = [iterate_user.name + ' ' + iterate_user.lastname];

              for (var i = 1; i < layout.cols.length; i++) {
                row[i] = summary[i].count;
              }
      ...
      Promise.all(promises_user_leave_summaries).then(function () {

        res.attachment(
          'leave_summary_' + month_from.format('YYYY-MM') + '.xlsx'
        );
        ...
        res.send(new Buffer(excel.execute(sheets), 'binary'));
